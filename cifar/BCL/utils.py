...
def cross_validation(x_train, y_train, random_state = 0):
    ...
    lr_cv = GridSearchCV(estimator = LogisticRegression(), param_grid = grid, cv=3, n_jobs=-1, verbose = 3)
...
def confusion_undersampled(file_format, epochs = 500, undersample = 10, model_seed = 6517725, test_size = 0.25, test_seed = 0, train_undersample = False):
    ...
    file = file_format + '_epoch_{}_undersample_{}_by_{}_seed_{}.npy'.format(epochs, undersample, underrepresentation,
                                                                             model_seed)
...
def indirect_effects(file_format, epochs = 500, model_seeds = [6517725, ], test_size = 0.25, test_seeds = [6517725, ], plot_classes = [0, 8, 1, 9,], figsize = (14, 4), filename = None):
